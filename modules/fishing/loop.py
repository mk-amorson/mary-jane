...
async def _step_reel(state, ctx, frame_np):
    ...
    # Always send gamepad update every tick (game needs continuous HID reports)
    if wanted is not None:
        key_down(wanted)
    ...
    if take:
        ...
        cx = tx + tw // 2
        cy = ty + th // 2
        log.info("%s  REEL: TAKE visible → click (%d,%d)", _ts(), cx, cy)
        click_at(cx, cy)
        _enter_step(state, ctx, "end")
...
async def _step_end(state, ctx, frame_np):
    ...
    if take:
        ...
        cx = tx + tw // 2
        cy = ty + th // 2
        log.info("%s  END: clicking TAKE at (%d,%d)", _ts(), cx, cy)
        click_at(cx, cy)
        ctx.take_clicked = True
        state.fishing2_take_pause = time.monotonic() + 3.0
        state.fishing2_take_icon = None
